...
math_utils_obj = real_time_manipulator_math_utils.manipulator_math_utils(
    JOINTS)
timestamps = []
angles = []
...
once = True
cats_transform = [[1, 180], [1, 180], [-1, 180],
                  [1, 180]]  # current angle to steps (cats)
...
def on_message(client, userdata, message):
    global start_time, count, init_timestamp, init_angle, time_diff, db, math_utils_obj, timestamps, angles, torques, padded_angles, first_val, mqtt_initval_error, gripper_state, once, cats_transform
    print(time.time() - start_time)
    start_time = time.time()
    msg = message.payload.decode("utf-8")
    # print("q.get_start", time.time() - preprocessing_time)
    msg = json.loads(msg)
    # print("q.get_end", time.time() - preprocessing_time)
