...
#define i2c_read(a, b, c, d) i2c_read_from_slave(a, b, c, d)
#define delay_ms esp32_delay_ms
#define get_ms esp32_get_clock_ms

static const char *TAG_INV_MPU = "inv_mpu";
...
// NOTE: This define the settings for getting the biases.
const struct test_s test = {
    .gyro_sens = 32768 / 250,
...
int mpu_reg_dump(void)
{
    ...
    for (ii = 0; ii < st.hw->num_reg; ii++)
    {
        ...
        if (i2c_read(st.hw->addr, ii, 1, &data))
            return -1;
        ESP_LOGI(TAG_INV_MPU, "%#5x: %#5x", ii, data);
    }
    return 0;
}
...
int mpu_init(void)
{
    ...
    if (mpu_set_compass_sample_rate(100))
        return -1;
    ESP_LOGI(TAG_INV_MPU, "MPU9250 Setup!");
#else
                  ...
                  uint8_t *sensors, uint8_t *more)
{
    ...
    if (fifo_count < packet_size)
        return 0;
    //    ESP_LOGI(TAG_INV_MPU, "FIFO count: %hd", fifo_count);
...
int mpu_set_bypass(uint8_t bypass_on)
{
    ...
    if (bypass_on)
    {
        ...
        if (i2c_read(st.hw->addr, st.reg->int_pin_cfg, 1, &tmp))
            return -1;
...
#endif
#ifdef AK89xx_SECONDARY
// NOTE: This function was modified according to suggestions in the Invensense Self-Test document.
// Sensitivity adjustments were found missing
static int compass_self_test(void)
{
    ...
    result = 0;
    /*    
    //NOTE: As given in Motion Driver 5.12
    #if defined MPU9150
...
*/
    ...
    // NOTE: As given in the Invensense Self-Test document.
    float __data[3];
    __data[0] = (((int16_t)(tmp[1] << 8) | tmp[0]) * st.chip_cfg.mag_sens_adj[0]) >> 8;
    __data[1] = (((int16_t)(tmp[3] << 8) | tmp[2]) * st.chip_cfg.mag_sens_adj[1]) >> 8;
    __data[2] = (((int16_t)(tmp[5] << 8) | tmp[4]) * st.chip_cfg.mag_sens_adj[2]) >> 8;

#if defined MPU9150
    if ((__data[0] > 100) || (__data[0] < -100))
        result |= 0x01;
    if ((__data[1] > 100) || (__data[1] < -100))
        result |= 0x02;
    if ((__data[2] > -300) || (__data[2] < -1000))
        result |= 0x04;
#elif defined MPU9250
    if ((__data[0] > 200) || (__data[0] < -200))
        result |= 0x01;
    if ((__data[1] > 200) || (__data[1] < -200))
        result |= 0x02;
    if ((__data[2] > -800) || (__data[2] < -3200))
        result |= 0x04;
#endif
...
static int accel_6500_self_test(int32_t *bias_regular, int32_t *bias_st, int debug)
{
    ...
    {
        if (debug)
            ESP_LOGI(TAG_INV_MPU, "Reading OTP Register Error.");
        return 0x07;
    }
    if (debug)
        ESP_LOGI(TAG_INV_MPU, "Accel OTP:%d, %d, %d", regs[0], regs[1], regs[2]);
    for (i = 0; i < 3; i++)
    {
        if (regs[i] != 0)
        {
    ...
    {
        if (debug)
            ESP_LOGI(TAG_INV_MPU, "ACCEL:CRITERIA A");
        for (i = 0; i < 3; i++)
        {
            st_shift_cust[i] = bias_st[i] - bias_regular[i];
            if (debug)
            {
                ESP_LOGI(TAG_INV_MPU, "Bias_Shift=%7.4f, Bias_Reg=%7.4f, Bias_HWST=%7.4f",
                         st_shift_cust[i] / 1.f, bias_regular[i] / 1.f,
                         bias_st[i] / 1.f);
                ESP_LOGI(TAG_INV_MPU, "OTP value: %7.4f", ct_shift_prod[i] / 1.f);
            }

            st_shift_ratio[i] = st_shift_cust[i] / ct_shift_prod[i] - 1.f;

            if (debug)
                ESP_LOGI(TAG_INV_MPU, "ratio=%7.4f, threshold=%7.4f", st_shift_ratio[i] / 1.f,
                         test.max_accel_var / 1.f);

            if (fabs(st_shift_ratio[i]) > test.max_accel_var)
            {
                if (debug)
                    ESP_LOGI(TAG_INV_MPU, "ACCEL Fail Axis = %d", i);
    ...
    else
    {
        ...
        if (debug)
        {
            ESP_LOGI(TAG_INV_MPU, "ACCEL:CRITERIA B");
            ESP_LOGI(TAG_INV_MPU, "Min MG: %7.4f", accel_st_al_min / 1.f);
            ESP_LOGI(TAG_INV_MPU, "Max MG: %7.4f", accel_st_al_max / 1.f);
        }
        ...
        for (i = 0; i < 3; i++)
        {
            ...
            if (debug)
                ESP_LOGI(TAG_INV_MPU, "Bias_shift=%7.4f, st=%7.4f, reg=%7.4f", st_shift_cust[i] / 1.f, bias_st[i] / 1.f, bias_regular[i] / 1.f);
            if (st_shift_cust[i] < accel_st_al_min || st_shift_cust[i] > accel_st_al_max)
            {
                if (debug)
                    ESP_LOGI(TAG_INV_MPU, "Accel FAIL axis:%d <= 225mg or >= 675mg", i);
                result |= 1 << i; //Error condition
            }
        }
    }
    ...
    if (result == 0)
    {
        ...
        if (debug)
            ESP_LOGI(TAG_INV_MPU, "Accel:CRITERIA C: bias less than %7.4f", accel_offset_max / 1.f);
        for (i = 0; i < 3; i++)
        {
            if (fabs(bias_regular[i]) > accel_offset_max)
            {
                if (debug)
                    ESP_LOGI(TAG_INV_MPU, "FAILED: Accel axis:%d = %d > 500mg", i, bias_regular[i]);
                result |= 1 << i; //Error condition
            }
        }
    }
...
static int gyro_6500_self_test(int32_t *bias_regular, int32_t *bias_st, int debug)
{
    ...
    if (debug)
        ESP_LOGI(TAG_INV_MPU, "Gyro OTP:%d, %d, %d", regs[0], regs[1], regs[2]);
    ...
    {
        if (debug)
            ESP_LOGI(TAG_INV_MPU, "GYRO:CRITERIA A");
        ...
        for (i = 0; i < 3; i++)
        {
            ...
            {
                if (debug)
                    ESP_LOGI(TAG_INV_MPU, "Gyro Fail Axis = %d", i);
                result |= 1 << i; //Error condition
            }
        }
    }
    else
    {
        ...
        if (debug)
        {
            ESP_LOGI(TAG_INV_MPU, "GYRO:CRITERIA B");
            ESP_LOGI(TAG_INV_MPU, "Max DPS: %7.4f", gyro_st_al_max / 1.f);
        }
        ...
        for (i = 0; i < 3; i++)
        {
            ...
            {
                if (debug)
                    ESP_LOGI(TAG_INV_MPU, "GYRO FAIL axis:%d greater than 60dps", i);
                result |= 1 << i; //Error condition
            }
        }
    }
    ...
    if (result == 0)
    {
        ...
        if (debug)
            ESP_LOGI(TAG_INV_MPU, "Gyro:CRITERIA C: bias less than %7.4f", gyro_offset_max / 1.f);
        for (i = 0; i < 3; i++)
        {
            if (fabs(bias_regular[i]) > gyro_offset_max)
            {
                if (debug)
                    ESP_LOGI(TAG_INV_MPU, "FAILED: Gyro axis:%d = %d > 20dps", i, bias_regular[i]);
                result |= 1 << i; //Error condition
            }
        }
    }
...
static int get_st_6500_biases(int32_t *gyro, int32_t *accel, uint8_t hw_test, int debug)
{
    ...
    if (debug)
        ESP_LOGI(TAG_INV_MPU, "Starting Bias Loop Reads");
    ...
    if (debug)
        ESP_LOGI(TAG_INV_MPU, "Samples: %d", s);
    ...
    if (debug)
    {
        ESP_LOGI(TAG_INV_MPU, "Accel offset data HWST bit=%d: %7.4f %7.4f %7.4f", hw_test, accel[0] / 65536.f, accel[1] / 65536.f, accel[2] / 65536.f);
        ESP_LOGI(TAG_INV_MPU, "Gyro offset data HWST bit=%d: %7.4f %7.4f %7.4f", hw_test, gyro[0] / 65536.f, gyro[1] / 65536.f, gyro[2] / 65536.f);
    }
...
int mpu_run_6500_self_test(int32_t *gyro, int32_t *accel, uint8_t debug)
{
    ...
    if (debug)
        ESP_LOGI(TAG_INV_MPU, "Starting MPU6500 HWST!");
    ...
    if (debug)
        ESP_LOGI(TAG_INV_MPU, "Retrieving Biases");
    ...
    if (ii == tries)
    {
         ...
         */
        if (debug)
            ESP_LOGI(TAG_INV_MPU, "Retrieving Biases Error - possible I2C error");
    ...
    if (debug)
        ESP_LOGI(TAG_INV_MPU, "Retrieving ST Biases");
    ...
    if (ii == tries)
    {
        ...
        if (debug)
            ESP_LOGI(TAG_INV_MPU, "Retrieving ST Biases Error - possible I2C error");
    ...
    if (debug)
        ESP_LOGI(TAG_INV_MPU, "Accel Self Test Results: %d", accel_result);

    gyro_result = gyro_6500_self_test(gyro, gyro_st, debug);
    if (debug)
        ESP_LOGI(TAG_INV_MPU, "Gyro Self Test Results: %d", gyro_result);
    ...
    if (debug)
        ESP_LOGI(TAG_INV_MPU, "Compass Self Test Results: %d", compass_result);
    if (!compass_result)
        result |= 0x04;
#else
    result |= 0x04;
...
restore:
    if (debug)
        ESP_LOGI(TAG_INV_MPU, "Exiting HWST");
    /* Set to invalid values to ensure no I2C writes are skipped. */
    st.chip_cfg.gyro_fsr = 0xFF;
    st.chip_cfg.accel_fsr = 0xFF;
    st.chip_cfg.lpf = 0xFF;
...
static int setup_compass(void)
{
    ...
    {
        int result = i2c_read(akm_addr, AKM_REG_WHOAMI, 1, data);
    ...
    if (akm_addr > 0x0F)
    {
        ...
        ESP_LOGE(TAG_INV_MPU, "Compass not found");
        return -1;
    }
